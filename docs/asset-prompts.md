...
## 脱 GPT 後処理（2026-07-07 調査 + 導入）

「アセットが全部 ChatGPT っぽい」問題の調査結果（海外のデザイナー・ML 技術者の一次発信
+ 査読論文を多数ソースで敵対的検証したもの）の要点:

- **黄/セピア被り**（通称 piss filter）: gpt-image-1/4o 世代の実在欠陥。
  **プロンプトでは除去できない**（最終レンダリング段に焼き付く）。後処理での
  LAB B チャンネル中和が定石（TechRadar ほか。UnYellowGPT 等の専用ツール生態系が傍証）
- **過飽和・「典型的な絵」への均質化**: classifier-free guidance のモード収束が機構
  （Dieleman/DeepMind 解説、ICLR 2025 が過飽和の原因項を特定）
- **ワックス質感**: 生成モデルは高周波成分を系統的に落とす（NeurIPS 2020 以降複数の査読研究）。
  RGB ヒストグラムも実写より狭く尖る（AI 検出器の根拠。CoDA 論文）
- 棄却された俗説: 「RLHF が暖色を好む」「プロンプト書き換えが warm 語を注入」（検証 0-3）

対応（`npm run degpt`。tools/degpt-assets.ts → src/tools/symbol-pipeline.ts の degptImage）:

1. **WB 中和**: 白いはずの画素（明るく低彩度 = シールの白縁）の平均 a*/b* を測って全体から引く
2. **彩度 0.92 倍**: CFG 由来の盛りをひと回し戻す
3. **印刷テクスチャ**: 紙目グレイン（シード付き決定論）+ 45° 網点のドットゲイン。
   高周波欠落とヒストグラムの尖りを同時に崩す。目標様式（90 年代印刷シール）と物理的に同じ操作
4. 図柄は 32 色に再量子化（様式検証は従来どおり通る）。パネルはランダム紙目だと
   PNG が肥大するため WB + 彩度 + 網点のみ

処理は決定論（ファイル名からシード導出）。**元に戻すには該当コミットを revert するだけ**
（原画は git 履歴にある）。プロンプト側の対策としては `glossy` 系の語がワックス質感の
AI 癖を増幅するため、matte / flat inks / screen-printed 系へ寄せる余地がある（未適用）。
2026-07 適用時の実測: 図柄の黄被りは b* +0.6〜+3.8（bell/blank/seven_red が大きい）。
...
- **2026-07-06 生成AIによる赤7高解像度化 + マスコット挿入候補**:
  ...
  ```
- **2026-07-06 標準図柄 6 種の生成AIリマスター**: ユーザー指示「他のやつも全部作って。
  キャラ関係なくて良い」を受け、`bar / bell / replay / cherry / melon / blank` を
  Codex 組み込み image_gen（gpt-image 系、正確なモデル名はツールから非公開）で
  個別に高解像度リマスター。各図柄の既存 PNG を形状・役割参照として、キャラ要素なしで
  4 号機期の印刷シール調に寄せた。生成元は `tmp/imagegen/<id>-genai-source.png` に保存し、
  以下の条件で標準アセットへ差し替え。

  ```
  npm run symbols -- tmp/imagegen/<id>-genai-source.png --name <id> \
    --out src/ui/assets/symbols --size 160 --aspect 1.6 --colors 32 --tolerance 100
  ```

  検証結果:
  - `bar`: 160x100px / 32 色 / 不透明率 39%
  - `bell`: 160x100px / 32 色 / 不透明率 46%
  - `replay`: 160x100px / 32 色 / 不透明率 52%
  - `cherry`: 160x100px / 32 色 / 不透明率 45%
  - `melon`: 160x100px / 32 色 / 不透明率 47%
  - `blank`: 160x100px / 31 色 / 不透明率 46%

  共通プロンプト方針:
  ```
  Use the current symbol image as the strict reference for subject,
  proportions, and layout. Create a high-resolution faithful remaster:
  premium 1990s Japanese pachislo printed sticker art, thick white die-cut
  border, bold black keyline, 2-3 tone cel shading, glossy screen-print
  highlights, centered on a flat #d9d9d9 background. Improve print quality
  without changing the symbol identity.
  ```
...
## 2026-07-19 PCモチーフ版（640x400）

サルドラちゃんのPCモチーフ（マウスカーソル、砂時計、Windows XP期のUI）を
役図柄へ展開。Codex組み込み ImageGen で各図柄を個別生成し、高解像度のまま差し替えた。

- `bell`: 黄金のXP風フォルダの中に金色ベル。クラッパーは砂時計、赤いリボン付き
- `replay`: 淡いシアンの透明ごみ箱、大きな緑の循環矢印、白いカーソルピン
- `melon`: 青と緑の地球儀、シアンの軌道がマウスカーソルになる。内部IDは互換性のため維持
- `seven_red`: 従来の赤7の外形とサルドラちゃんの構図を保った高精細リマスター
- `bar`: 従来の金・バーガンディの額縁と `BAR` 文字を保った高精細リマスター
- `blank`: Windows 95期の汎用EXEをモチーフにしたオリジナル図柄。折り返し付き用紙、
  丸インジケーター1個の青いタイトルバー、壊れたピクセル片3個、砂時計カーソル

生成時は平坦な高彩度クロマキー背景を指定し、背景除去後に比率を保って中央配置する。
リール表示でのガサつきを避けるため、旧パイプラインの160x100縮小、32色量子化、
アルファ2値化は行わない。

```powershell
npm run symbols:prepare -- --input <alpha.png> --output <symbol.png> --width-use 0.9 --height-use 0.88
```
